...
/* -------- Collapsible category (CSS grid-rows animation) -------- */
...
export default function SkillGuideModal({ guide, onClose }) {
    ...
    <AnimatePresence>
      {guide && (
        <motion.div
          ...
          role="dialog"
          aria-modal="true"
          aria-label={`${guide.name} reference guide`}
        >
          {/* Backdrop */}
          ...
          <motion.div
            ...
            className="glass-strong relative flex max-h-[92vh] w-full max-w-3xl flex-col overflow-hidden rounded-t-3xl border-border-strong sm:rounded-3xl"
          >
            {/* Header */}
            <div className="flex items-start justify-between gap-4 border-b border-border p-6 pb-5">
              ...
              <button
                ...
                onClick={onClose}
                aria-label="Close"
                className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-border bg-surface/60 text-muted transition-colors hover:text-text"
              >
                <HiXMark size={20} />
              </button>
            </div>
